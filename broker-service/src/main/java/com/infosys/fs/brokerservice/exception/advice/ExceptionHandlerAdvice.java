package com.infosys.fs.brokerservice.exception.advice;
...
import com.infosys.fs.brokerservice.util.StatusCodes;
...
public class ExceptionHandlerAdvice {
	...
	/*	*//**
			 * Method to handle Hystrix RuntimeException which would be thrown
			 * when the circuit circuit is OPEN.
			 *
			 * @param exception
			 *            - HystrixRuntimeException runtime being handled
			 * @return ResponseEntity<Object> - Response entity being returned
			 *//*
				 * @ExceptionHandler(HystrixRuntimeException.class) public
				 * ResponseEntity<Object>
				 * handleHystrixRuntimeException(HystrixRuntimeException
				 * exception) { LOGGER.error("Hystrix Runtime Exception ",
				 * exception);
				 * LOGGER.error("Inside handleHystrixRuntimeException ",
				 * exception); APIResponseView responseView =
				 * this.mapResponseView(StatusCodes.EXTERNAL_SYSTEM_UNAVAILABLE.
				 * getStatusCode(),
				 * StatusCodes.EXTERNAL_SYSTEM_UNAVAILABLE.getStatusDescription(
				 * ), Severity.CRITICAL);
				 * 
				 * return ResponseEntity.status(552).body(responseView); }
				 */
