...
import com.infosys.fs.paymentprocessingsys.config.HttpClientConfig.HttpHostConfiguration;
...
public class CustomRestTemplate {
		...
		return new ConnectionKeepAliveStrategy() {
			...
			public long getKeepAliveDuration(HttpResponse response, HttpContext context) {
				...
				while (it.hasNext()) {
					...
					if ((value != null) && StringUtils.equalsAnyIgnoreCase(param, "timeout")) {
	...
	public static <T> ResponseEntity<T> invokeRestAPI(String url, HttpMethod httpMethod, Class<T> responseClass,
			...
			List<MediaType> acceptableMediaTypes, RestTemplate rest) throws BadRequestException, NotFoundException,
			InternalServerErrorException, NotImplementedException, ExternalSystemUnavailableException {
		...
		try {
			...
			throw new InternalServerErrorException(StatusCodes.EXTERNAL_SYSTEM_CONNECTION_ISSUE.getStatusCode(),
					StatusCodes.EXTERNAL_SYSTEM_CONNECTION_ISSUE.getStatusDescription(), Severity.CRITICAL);
		}
		if (null != response) {
